...
class AutoencoderTrainer:
    # Training and utility methods for Autoencoder
    ...
    def preprocessing(self, data, test_size=0.2, batch_size=256):
        # Scale and prepare data loaders
        n_samples = len(data)
...
def autoencoder_workflow(train_df, encoding_dim=None, epochs=100):
    ...
    encoded_df = pd.DataFrame(encoded_features, columns=[f'encoded_feature_{i}' for i in range(encoding_dim)], index=train_df.index)
